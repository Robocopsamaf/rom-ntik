...
export async function analyzeBatch(texts: string[], lang: string): Promise<AnalyzedSentence[]> {
  const need: string[] = [];
  const indexMap: number[] = [];
  texts.forEach((t, i) => {
    // Cache key is namespaced by language so the same surface form analysed in
    // two source languages doesn't collide.
    if (!cache.has(`${lang}|${t}`)) {
      need.push(t);
      indexMap.push(i);
    }
  ...
  if (need.length > 0) {
    const resp = await fetch(`${backendUrl}/analyze`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sentences: need, lang }),
    });
    if (!resp.ok) throw new Error(`analyze failed: ${resp.status}`);
    const data = (await resp.json()) as AnalyzeResponse;
    data.sentences.forEach((s, k) => cache.set(`${lang}|${need[k]}`, s));
  }
  return texts.map((t) => cache.get(`${lang}|${t}`)!);
}
