...
export async function findSourceTrack(sourceLang: string): Promise<CaptionTrackInfo | null> {
  for (let i = 0; i < 20; i++) {
    const pr = readPlayerResponse();
    const tracks = pr?.captions?.playerCaptionsTracklistRenderer?.captionTracks;
    if (tracks && tracks.length > 0) {
      // Match the configured source language. YouTube codes can be regional
      // (e.g. "pt-BR", "es-419"), so match on the base language subtag too.
      const base = (t: CaptionTrackRaw) => t.languageCode.split("-")[0].toLowerCase();
      const matches = tracks.filter((t) => base(t) === sourceLang.toLowerCase());
      if (matches.length === 0) return null;
      const manual = matches.find((t) => t.kind !== "asr");
      const pick = manual ?? matches[0];
      return {
...
function parseBody(body: string): Cue[] {
  ...
  try {
    ...
    console.warn("[romäntik] caption parse failed", e);
    return [];
  }
}
...
export function requestCues(
  ...
  return new Promise((resolve) => {
    ...
    function handler(ev: MessageEvent) {
      ...
      window.removeEventListener("message", handler);
      console.log("[romäntik] captions resp", { srcBytes: (d.ru ?? "").length, trBytes: (d.tr ?? "").length });
      resolve({ src: parseBody(d.ru ?? ""), tr: parseBody(d.tr ?? "") });
    }
