...
import { DEFAULT_SETTINGS, LANGS, SOURCE_LANGS, Settings } from "./types";
...
export function buildSettingsForm(initial: Settings): BuiltForm {
  ...
  const srcWrap = el("label", { class: "sr-panel-field" });
  srcWrap.appendChild(el("span", {}, "Spoken language"));
  const srcSel = el("select", { name: "sourceLang" }) as HTMLSelectElement;
  for (const { code, label } of SOURCE_LANGS) {
    srcSel.appendChild(el("option", { value: code }, `${label} (${code})`));
  }
  srcSel.value = initial.sourceLang;
  ...
  const toggles: { key: "showColors" | "showTooltips"; label: string }[] = [
