...
type TranslateWord = (word: string) => Promise<string>;
...
function format(target: HTMLElement) {
  const surface = target.dataset.surface ?? target.textContent ?? "";
  ...
  // not the inflected surface — an isolated inflected form is often mis-sensed
  // (e.g. "annoncé" → "announcement" instead of the verb "annoncer" → "announce").
  const seq = ++hoverSeq;
